...
def train(rank, opt):
    ...
    if 'IF' in opt.guidance:
        ...
        guidance['IF'] = IF(device, opt.vram_O, opt.is_BSD, opt.t_range)
    ...
    if opt.num_gpus > 1:
        trainer = Trainer(' '.join(sys.argv), 'DreamInit', opt, model, guidance, device=device, workspace=opt.workspace,
    ...
    if 'IF' in opt.guidance:
        guidance[opt.guidance[0]].unload_encoder()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--test_interval', type=int, default=2, help="test on the test set every interval epochs")
    ...
    parser.add_argument('--grad_clip', type=float, default=5.0, help="clip grad of all grad to this limit, negative value disables it")
    ...
    parser.add_argument('--batch_size', type=int, default=1, help="batch_size of prompts")
    ...
    parser.add_argument('--fixed_scales', type=float, default=-4.0)
    parser.add_argument('--grid_resolution', type=int, default=48)
    parser.add_argument('--patch_size', type=int, default=12)
    parser.add_argument('--hidden_dim', type=int, default=128)

    parser.add_argument('--is_BSD', action='store_true', help="use the BSD loss")
