...
class GasussianDataset:
    def __init__(self, opt, device, guidance, type='train'):
        ...
        with torch.no_grad():
            embeddings['default'] = guidance.get_text_embeds([self.opt.prompt]).to(self.device)
            embeddings['uncond'] = guidance.get_text_embeds(['']).to(self.device)
            
        for d in ['front', 'side', 'back']:
            embeddings[d] = guidance.get_text_embeds([f"{self.opt.prompt}, {d} view"]).to(self.device)

        if self.training:
            self.texts = [self.opt.prompt] * 100
            self.texts_embeddings = [embeddings] * 100
        else:
            self.texts = [self.opt.prompt]
            self.texts_embeddings = [embeddings]
    ...
    def get_text_embeddings_gpu(self, index):
        ...
        texts_embedding['uncond'] = self.texts_embeddings[index]['uncond'].to(self.device)
        for d in ['front', 'side', 'back']:
    ...
    def collate(self, index):
        ...
        for i in range(B):
            if self.training:
                ...
                eval_cameras = GenerateCircleCameras(self.opt, self.num_view, render45=True)
